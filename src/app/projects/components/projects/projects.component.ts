  ...
  createDefaultOrg(user) {
    ...
    if (!this.isManaged && !ConfigManager.getIsDemo()) {
      this.subscriptions$.push(this.organizationApolloService.canCreateLocalOrg().pipe(first()).subscribe((canCreateOrg) => {
        ...
        if (canCreateOrg) {
          this.organizationApolloService.createOrganization().pipe(first()).subscribe(
            (Orgname) => this.organizationApolloService.addUserToOrganization(user.mail, Orgname.organization.name).pipe(first()).subscribe(
